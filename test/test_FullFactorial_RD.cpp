...
int main()
{
	...
	auto varX=block->addParameter("x", vector<double>{0, .11, .33, .44, .77, .99, 1});
	auto varY=block->addParameter("y", -1, 1);
	auto varZ=block->addParameter("z", 0, 10);

	varX->variance(.08, AnalysisParameter::varianceType::PERCENTAGE_);
	varY->variance(1, AnalysisParameter::varianceType::ABSOLUTE_);
	varZ->variance(1.3, AnalysisParameter::varianceType::ABSOLUTE_);
	...
	model.setObjf(objf);
	model.setRobustDesign(true,2000);
	model.run();
	model.dumpSamples(R"(C:\tmp\samples_FF_RD.txt)", 0);// , OPTIPP_DUMP_FEAS | OPTIPP_DUMP_PARETO);
	block->evalCorrCoeff();
	return 0;
}